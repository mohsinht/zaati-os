...
test("generates a complete multi-source scheduled task from live contracts", () => {
  ...
  assert.match(promptText, /Actions and Workflows write permission is forbidden/)
  assert.ok(promptText.split("\n").length < 300, "generated prompt should stay readable")
  assert.match(artifacts.files["daily-calm-pulse.permissions.md"], /human-readable permission receipt|permission manifest/i)
  assert.match(artifacts.files["daily-calm-pulse.data-repository.md"], /--sources money:pulse,news:briefing/)
})
...
test("writes private artifacts and refuses accidental replacement", async () => {
  ...
  assert.equal((await writePromptArtifacts(artifacts, directory, { force: true })).length, 4)
})
