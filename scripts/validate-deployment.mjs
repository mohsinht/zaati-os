...
if (config.account_id || config.routes || config.route) errors.push("Account IDs and personal hostnames belong in generated deployment configuration, not the public base file.")
try { await access("public/_redirects"); errors.push("public/_redirects must not exist because Wrangler owns SPA routing.") } catch (error) { if (error.code !== "ENOENT") throw error }
for (const requirement of ["Content-Security-Policy:", "frame-ancestors 'none'", "X-Content-Type-Options: nosniff", "X-Robots-Tag: noindex", "/data/*", "Cache-Control: private, no-store"]) {
