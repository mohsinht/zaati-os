...
import { expandDependencies } from "./lib/setup-options.mjs"
...
const codeRef = option("code-ref")
if (!targetOption) throw new Error("Provide the private data repository directory with --target.")
if (!codeRef) throw new Error("Provide the immutable validator revision with --code-ref FULL_COMMIT_SHA.")
...
const requestedSources = option("sources")
  ?.split(",")
  .map((item) => item.trim())
  .filter(Boolean)
let workflowId = option("workflow") || "daily-core"
let expectedSourceIds
if (requestedSources?.length) {
  const unknown = requestedSources.filter((id) => !registry.sources.some((source) => source.id === id))
  if (unknown.length) throw new Error(`Unknown sources: ${unknown.join(", ")}.`)
  expectedSourceIds = expandDependencies(requestedSources, registry)
  const matching = workflows.workflows.find(
    (item) => item.source_ids.length === expectedSourceIds.length && item.source_ids.every((id, index) => id === expectedSourceIds[index]),
  )
  workflowId = matching?.id || "custom"
} else {
...
const workflowText = template.replaceAll("YOUR_USER/zaati-os", codeRepository).replaceAll("VALIDATOR_CODE_REF", codeRef)
const config = {
...
console.log(
  `Prepared an independent ${workflowId} validation gate for ${expectedSourceIds.join(", ")} in ${target}. Review, commit, and require its check on main.`,
)
