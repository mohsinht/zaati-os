...
const selected = output.filter(
  (file) => referenced.has(path.relative(dist, file).replaceAll(path.sep, "/")) || file.endsWith("dashboard-data.json"),
)
const sizes = await Promise.all(
  selected.map(async (file) => ({ file, raw: (await stat(file)).size, gzip: gzipSync(await readFile(file)).length })),
)
...
console.log(
  `Performance budgets: JavaScript ${Math.ceil(results.totalJs / 1024)} KB gzip, CSS ${Math.ceil(results.css / 1024)} KB gzip, dashboard data ${Math.ceil(results.data / 1024)} KB gzip.`,
)
