...
const protectedByDenial = [401, 403].includes(response.status) && /cloudflare|access/i.test(`${response.headers.get("server") || ""} ${await response.text().catch(() => "")}`)
if (!protectedByRedirect && !protectedByDenial) {
  throw new Error(`Access preflight failed for ${hostname}. Expected an Access redirect or denial, received HTTP ${response.status}. Do not deploy private snapshots.`)
}
