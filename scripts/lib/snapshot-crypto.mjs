...
export async function loadSnapshotKey({ keyFile = process.env.ZAATI_SNAPSHOT_KEY_FILE || ".zaati/snapshot.key" } = {}) {
  ...
  const value = await readFile(path.resolve(keyFile), "utf8").catch((error) => {
    if (error.code === "ENOENT") throw new Error("Snapshot encryption is enabled, but no key was found. Set ZAATI_SNAPSHOT_KEY or run npm run snapshot:keygen.")
    throw error
  })
...
export function decryptSnapshotEnvelope(envelope, value) {
  ...
  if (!envelope || envelope.format !== FORMAT || envelope.version !== VERSION || envelope.algorithm !== ALGORITHM || keys.join(",") !== expectedKeys.join(",")) {
    throw new Error("Unsupported encrypted snapshot envelope.")
  }
  if (!/^[a-f0-9]{12}$/.test(envelope.key_id) || !/^[A-Za-z0-9_-]{16}$/.test(envelope.iv) || !/^[A-Za-z0-9_-]{22}$/.test(envelope.auth_tag) || !/^[A-Za-z0-9_-]+$/.test(envelope.ciphertext) || envelope.ciphertext.length > 8_000_000) throw new Error("Malformed encrypted snapshot envelope.")
  const key = decodeKey(value)
